...
/**
 * Concrete implementation of {@link DriverFactory} backed by
 * Playwright. This factory manages browser instances per browser
 * type and creates new browser contexts for each page request to
 * ensure isolation between tests. Video recording is configured
 * according to the provided {@link DriverConfig}.
 */
@Slf4j
@Component
public class PlaywrightDriverFactory implements DriverFactory {
    ...
    public synchronized Page createPage(BrowserType browserType, DriverConfig config) {
        ...
        Browser browser = browsers.computeIfAbsent(browserType, type -> {
