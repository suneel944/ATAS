...
/**
 * Service that tracks the progress of running test executions and
 * notifies subscribers via Server-Sent Events (SSE).  It exposes
 * methods to retrieve the current status of a test execution as
 * well as to register SSE clients for live updates.  A scheduled
 * task periodically sends updates to all active emitters.
 */
@Service
...
public class TestMonitoringService {
    ...
    /**
     * Get the current status of the specified execution.  This method
     * queries the database for the execution and its results and
     * aggregates statistics such as the number of passed/failed tests
    ...
    /**
     * Register an SSE emitter for the given execution.  The emitter
     * will receive periodic updates until it times out or completes.
     *
    ...
    /**
     * Scheduled task that broadcasts execution status updates to
     * registered SSE emitters.  Executions with no active emitters
     * are skipped.  This method runs every second by default.
     */
