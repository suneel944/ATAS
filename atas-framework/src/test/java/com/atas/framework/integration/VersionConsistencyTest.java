...
/**
 * Unit test to detect version mismatches between Docker Compose and integration test configurations.
...
class VersionConsistencyTest {
    ...
    void shouldHaveConsistentPostgreSQLVersions() {
        ...
        assertThat(integrationTestVersion)
                .withFailMessage("PostgreSQL version mismatch detected! " +
    ...
    void shouldUseSupportedPostgreSQLVersions() {
        ...
        assertThat(dockerMajorVersion)
                .withFailMessage("Docker Compose PostgreSQL version %s is not supported (minimum: 12)", dockerComposeVersion)
                .isGreaterThanOrEqualTo(12);
                
        assertThat(integrationMajorVersion)
                .withFailMessage("Integration test PostgreSQL version %s is not supported (minimum: 12)", integrationTestVersion)
                .isGreaterThanOrEqualTo(12);
    }
    ...
    private String extractPostgreSQLVersionFromDockerCompose() {
        try {
            ...
            java.nio.file.Path dockerComposePath = java.nio.file.Paths.get(
                System.getProperty("user.dir"), "..", "docker", "docker-compose-local-db.yml").normalize();
            
            if (!java.nio.file.Files.exists(dockerComposePath)) {
                throw new IllegalStateException("Could not find docker-compose-local-db.yml file at: " + dockerComposePath);
            }
            ...
            throw new IllegalStateException("Failed to extract PostgreSQL version from Docker Compose", e);
        }
    }
    ...
    private String extractPostgreSQLVersionFromIntegrationTest() {
        try {
            ...
            java.nio.file.Path integrationTestPath = java.nio.file.Paths.get(
                System.getProperty("user.dir"), "src", "test", "java", 
                "com", "atas", "framework", "integration", "DatabaseIntegrationTest.java").normalize();
            
            if (!java.nio.file.Files.exists(integrationTestPath)) {
                throw new IllegalStateException("Could not find DatabaseIntegrationTest.java file at: " + integrationTestPath);
            }
            ...
            if (matcher.find()) {
                ...
                throw new IllegalStateException("PostgreSQL container declaration not found in integration test");
            }
            
        } catch (Exception e) {
            throw new IllegalStateException("Failed to extract PostgreSQL version from integration test", e);
        }
    }
