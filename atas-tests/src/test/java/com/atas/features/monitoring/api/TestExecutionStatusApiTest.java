...
/**
 * API tests for test execution status monitoring endpoints.
 * Tests various scenarios for status retrieval and aggregation.
 */
@SpringBootTest(classes = AtasFrameworkApplication.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@ActiveProfiles("test")
@Slf4j
class TestExecutionStatusApiTest {
    ...
    void statusEndpointReturnsCorrectCountsForMixedResults() {
        ...
        String url = String.format("http://localhost:%d/api/v1/test-execution/status?executionId=%s", 
